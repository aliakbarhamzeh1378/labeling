...
def draw_boxes(image, boxes, labels):
    ...
    for box in boxes:
        ...
        sObj = labels[box.get_label()] + ' '
        if (box.get_score() > 0.10):
            ObjectsFound.append([labels[box.get_label()], xmin, ymin, box.get_score()])
            cv2.putText(image, sObj, (xmin, ymin - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.25, (0, 255, 0), 1)

    # ObjectsFound = np.array(ObjectsFound)

    ObjectsFound = sorted(ObjectsFound, key=lambda l: l[1])
    ObjectsFound = np.array(ObjectsFound)
    return image, ObjectsFound
